...
**Status:** Deprecated by ADR-015 — everything it decided is gone. The zoom ladder went with `IsometricCamera` and the starfield densities with `Starfield` (2026-09-10); `GLYPH_SCALE` had already been revised into a per-call argument by ADR-014. The principle -- that a change of resolution must not silently rescale the picture -- is the part worth keeping.

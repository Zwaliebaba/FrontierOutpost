...
**Status:** Accepted — reaffirmed by ADR-034 (2026-09-11) against `Design/UI/DESIGN-GUIDELINES.md`, which restates the authored projection this ADR replaced. That formula describes the mockups; the camera is the map. Its "there is no matrix" clause is revised by ADR-103 (2026-09-14): the camera now also states itself as a matrix for the mesh pass, held equal to `Project` by test.

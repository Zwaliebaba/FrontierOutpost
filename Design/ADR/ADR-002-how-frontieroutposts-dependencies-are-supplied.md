...
| **FMOD Ex 4.44** | Prebuilt and proprietary; there is no other way. x86 as in the original (`extlib/win32`, `extbin/win32`, unused by FrontierOutpost's platforms). **x64 supplied by the owner** into `extlib/x64/FMOD` and `extbin/x64`. The tree carries no FMOD licence text, so the owner's supply should include FMOD's terms. | owner-supplied | **none: documented blocker** (MIGRATION_NOTES.md D9) |

...
1. **The tree becomes an instruction stream:** postfix, opcode and parameters, in a structured
   buffer. An instruction encoder in liblt writes it where `GetCode` and `FIELDFN` did (plan §5.4,
   §5.7).

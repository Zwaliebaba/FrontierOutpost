...
- Projection (design space 800×560 → screen): for a node at `(x, y)`, `d = y/560`, `s = 0.5 + 0.65·d`, `sx = 400 + (x−400)·s`, `sy = 60 + 440·(0.3·d + 0.7·d²)`. Node size, label size and stem height scale with `s`. Use `preserveAspectRatio: meet` (letterbox, never crop).
...
- Map: pan/zoom; perspective is fixed (no free camera). Optional slow pulse (`fo-pulse`, opacity .35↔1) on fleets with ETA = next tick.

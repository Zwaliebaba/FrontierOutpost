...
**R13 binds a process acting as the CLIENT, and has exactly one sanctioned exception** (ADR-024 and ADR-028, owner decisions, 2026-09-11). A process **acting as the server** may write **one match store**: the rules, the seed, and every locked order set, from which a match is loaded by re-resolving it. Nothing else, and never a client.

**It is a role and not a binary**, because there is one executable and it has three of them (ADR-028): `--serve` and the default host-and-play act as the server and may write a store; `--join` is only a client and must not. The exception is also named rather than general — the next thing that wants to write a file is a new decision, not an inference from this one. The shipped executable still needs nothing beside it: art, colours, fonts and compiled shaders are all embedded, and a client reads nothing at all.

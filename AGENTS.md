...
**R13 binds the shipped client, and has exactly one sanctioned exception** (ADR-024, owner decision, 2026-09-11). A server process may write **one match store**: the rules, the seed, and every locked order set, from which a match is loaded by re-resolving it. Nothing else, and not the client — `FrontierOutpost.exe` still ships alone, reads nothing, and embeds its font, its colours and its shaders. This exception is named rather than general: the next thing that wants to write a file is a new decision, not an inference from this one.

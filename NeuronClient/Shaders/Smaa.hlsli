// FrontierOutpost/src/liblt/Shaders/Smaa.hlsli
//
// GameData/shader/common/smaa.jsl, in HLSL: SMAA, by Jorge Jimenez, Jose I. Echevarria, Belen Masia,
// Fernando Navarro and Diego Gutierrez, under the MIT licence in SmaaLicense.txt beside this file.
// Only its porting block is this engine's; the GLSL port that came with the game changed the rest
// as its diff with smaa.jsl shows, and this changes it no further.
...
// Porting Functions
//
// SMAA's HLSL 4 porting block, with one change for this engine. SMAA's own block samples every
// texture through two fixed samplers, linear and point. This version of SMAA was adapted to GLSL
// before it came here: it samples the search texture through SMAASample too, relying on that
// texture's own nearest filter (see SMAASearchLength). So a texture travels with the sampler GL
// gave it. As in SMAA's HLSL 4 block, every sample is at level zero, which a loop needs.
struct SMAATexture2D {
  Texture2D image;
  SamplerState state;
};
#define SMAASample(tex, coord) tex.image.SampleLevel(tex.state, coord, 0.0)
#define SMAALerp(a, b, t) lerp(a, b, t)
#define SMAAMad(a, b, c) mad(a, b, c)
#define SMAASaturate(a) saturate(a)

// A texture declared with TEXTURE2D, and its sampler, as one SMAATexture2D.
SMAATexture2D SMAATexturePair(Texture2D image, SamplerState state) {
  SMAATexture2D pair;
  pair.image = image;
  pair.state = state;
  return pair;
  ...
  float c)
{
  ...
  for (i = 0.0; i < float(SMAA_MAX_SEARCH_STEPS_DIAG); i++) {
      ...
      e.r = SMAASample(edgesTex, uv + rcpFrame * float2(1, 0)).r;
      if (dot(e, float2(1.0, 1.0)) < 1.9) break;
      uv += dir * SMAA_PIXEL_SIZE;
  }
  return i + float(e.g > 0.9) * c;
  ...
  int4 subsampleIndices)
{
  ...
  if (d.r + d.g > 2.0) { // d.r + d.g + 1 > 3
    ...
    float4 c;
    c.x = SMAASample(edgesTex, coords.xy + rcpFrame * float2(-1,  0)).g;
    c.y = SMAASample(edgesTex, coords.xy + rcpFrame * float2( 0,  0)).r;
    c.z = SMAASample(edgesTex, coords.zw + rcpFrame * float2( 1,  0)).g;
    c.w = SMAASample(edgesTex, coords.zw + rcpFrame * float2( 1, -1)).r;
  ...
  float right = SMAASample(edgesTex, uv + rcpFrame * float2(1, 0)).r;
  ...
  if (d.r + d.g > 2.0) { // d.r + d.g + 1 > 3
    ...
    float4 c;
    c.x  = SMAASample(edgesTex, coords.xy + rcpFrame * float2(-1,  0)).g;
    c.y  = SMAASample(edgesTex, coords.xy + rcpFrame * float2( 0, -1)).r;
    c.zw = SMAASample(edgesTex, coords.zw + rcpFrame * float2( 1,  0)).gr;
    float2 e = 2.0 * c.xz + c.yw;
    float t = float(SMAA_MAX_SEARCH_STEPS_DIAG) - 1.0;
    e *= step(d.rg, float2(t, t));
  ...
  float2 d)
{
  ...
  float2 e;
  e.r = SMAASample(edgesTex, coords.xy + rcpFrame * float2(0.0,  1.0)).r;
  bool left = abs(d.x) < abs(d.y);
  e.g = SMAASample(edgesTex, coords.xy + rcpFrame * float2(0.0, -2.0)).r;
  if (left) weights *= SMAASaturate(float(SMAA_CORNER_ROUNDING) / 100.0 + 1.0 - e);

  e.r = SMAASample(edgesTex, coords.zw + rcpFrame * float2(1.0,  1.0)).r;
  e.g = SMAASample(edgesTex, coords.zw + rcpFrame * float2(1.0, -2.0)).r;
  if (!left) weights *= SMAASaturate(float(SMAA_CORNER_ROUNDING) / 100.0 + 1.0 - e);
}
  ...
  float2 d)
{
  float4 coords = SMAAMad(float4(0.0, d.x, 0.0, d.y),
                          SMAA_PIXEL_SIZE.xyxy, uv.xyxy);
  float2 e;
  e.r = SMAASample(edgesTex, coords.xy + rcpFrame * float2( 1.0, 0.0)).g;
  bool left = abs(d.x) < abs(d.y);
  e.g = SMAASample(edgesTex, coords.xy + rcpFrame * float2(-2.0, 0.0)).g;
  if (left) weights *= SMAASaturate(float(SMAA_CORNER_ROUNDING) / 100.0 + 1.0 - e);

  e.r = SMAASample(edgesTex, coords.zw + rcpFrame * float2( 1.0, 1.0)).g;
  e.g = SMAASample(edgesTex, coords.zw + rcpFrame * float2(-2.0, 1.0)).g;
  if (!left) weights *= SMAASaturate(float(SMAA_CORNER_ROUNDING) / 100.0 + 1.0 - e);
}
...
{ // Just pass zero for SMAA 1x, see @SUBSAMPLE_INDICES.
  ...
  if (e.g > 0.0) { // Edge at north
    ...
    if (dot(weights.rg, float2(1.0, 1.0)) == 0.0) {
      ...
      float e2 = SMAASample(edgesTex, coords + rcpFrame * float2(1, 0)).r;
  ...
  if (e.r > 0.0) { // Edge at west
    ...
    float e2 = SMAASample(edgesTex, coords + rcpFrame * float2(0, 1)).g;
  ...
  SMAATexture2D blendTex)
{
  ...
  if (tw < 1e-5)
    ...
    float4 c = ow * toGamma(SMAASample(colorTex, uv));
    c += a.b * toGamma(SMAASample(colorTex, uv + SMAA_PIXEL_SIZE * float2(-1.0, 0.0)));
    c += a.a * toGamma(SMAASample(colorTex, uv + SMAA_PIXEL_SIZE * float2( 1.0, 0.0)));
    c += a.g * toGamma(SMAASample(colorTex, uv + SMAA_PIXEL_SIZE * float2( 0.0, 1.0)));
    c += a.r * toGamma(SMAASample(colorTex, uv + SMAA_PIXEL_SIZE * float2( 0.0,-1.0)));

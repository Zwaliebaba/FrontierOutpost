// PaletteResolveVS.hlsl -- the vertex half of the pass that turns palette indices into pixels.
//
// A fullscreen triangle generated from SV_VertexID: no vertex buffer, no index buffer and no
// input layout. Three vertices, one draw call, and nothing to keep in sync with a C++ struct.
// A quad would need two triangles and would rasterize the diagonal twice.

...
def check_generated_font() -> None:
    """NeuronClient/Font.h is written by Build/BakeFont.py, and this is what keeps it honest.

    ADR-073 bakes the font OFFLINE and commits the header, which buys a build that needs no
    rasterizer and costs the one thing a generated-and-committed artifact always costs: nothing
    forces it to match its source. So the baker records three SHA-256s in the header -- every source
    TTF, itself, and the generated text -- and this re-derives all three.

    IT DOES NOT RE-BAKE. Re-baking would need freetype-py on the machine running the check, which
    would put a Python font library in CI's path for every build and hand back exactly the standing
    tax ADR-073 rejected an MSBuild step to avoid. Hashing catches a swapped font, an edited baker
    and a hand-edited header; what it cannot catch is a rasterizer that stopped being
    deterministic, which is why the baker pins its parameters in the header it writes.

    The header is skipped entirely while it is still the hand-typed 8x8 font -- FONT-01 stage 1
    wrote the baker, and stage 2 is what makes its output the thing that ships.
    """
    header_path = os.path.join(REPO_ROOT, "NeuronClient", "Font.h")
    if not os.path.exists(header_path):
        fail("NeuronClient/Font.h: missing. It is generated -- run `py Build/BakeFont.py`.")
        return

    with open(header_path, "r", encoding="utf-8") as handle:
        header = handle.read()

    if "GENERATED BY Build/BakeFont.py" not in header:
        return  # still the hand-typed font; FONT-01 stage 2 is what changes that.

    for relative, digest in re.findall(r"//\s+source (\S+)\s+sha256 ([0-9a-f]{64})", header):
    ...
    if not stated:
        fail("NeuronClient/Font.h: generated, but records no content hash. Re-run `py Build/BakeFont.py`.")
        return
    without = header.replace(f"// Content sha256 {stated.group(1)}\n//\n", "", 1)
    if hashlib.sha256(without.encode("utf-8")).hexdigest() != stated.group(1):
        fail("NeuronClient/Font.h: edited by hand since it was baked. It is generated output "
             "(ADR-073); change Build/Fonts or Build/BakeFont.py and re-bake.")

...
/// The same border under the pointer (ADR-110). Twice the alpha, which is the smallest step that
...
/// (ADR-110). **A press has to be visible on the one control whose rest state is already the
...
/// The number segment's ground on a FILLED button (ADR-110). Black at a wash rather than a second
...
/// The ground the map's banner wears while a move is being chosen on it (ADR-113). `BLUE` at a
...
/// The border of a control that cannot be ordered, drawn DASHED (ADR-110).
///
...
/// The same wash under the pointer (ADR-110). A committed control's hover has to be visible while
